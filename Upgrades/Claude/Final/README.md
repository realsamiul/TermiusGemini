...
The previous files (Proposed1–3, OldGEMINI.md, Gemini_V2.md) had three problems:
...
**3. Fenced code blocks in markdown context.** The Proposed files contained fenced code blocks inside markdown that itself was being loaded as GEMINI.md context. Gemini's markdown parser treats content inside triple-backtick blocks as literal text, not instructions. Any protocol that was formatted as a code block (the SCRATCHPAD format, the REFLECTION checklist, the SPEC format) was being read as example text rather than behavioral instruction. The fixed GEMINI.md avoids this by keeping all formats as inline monospace or plain text, not fenced blocks.
